...
namespace Manina.Windows.Forms
{
    ...
    internal class ImageListViewCacheManager : IDisposable
    {
        ...
        private class CacheItem : IDisposable
        {
            ...
            /// <summary>
            /// Gets the date and time file modified of the requested thumbnail.
            /// </summary>
            ...
            /// <param name="state">The cache state of the item.</param>
            /// <param name="useEmbeddedThumbnails">UseEmbeddedThumbnails property of the owner control.</param>
            public CacheItem(Guid guid, string filename, DateTime dateModified, Size size, Image image, CacheState state, UseEmbeddedThumbnails useEmbeddedThumbnails)
            {
                mGuid = guid;
                mFileName = filename;
                mSize = size;
        ...
        public void BeginItemEdit(Guid guid)
        {
            ...
            lock (lockObject)
            {
                if (!editCache.ContainsKey(guid))
                {
                    ...
                    if (!string.IsNullOrEmpty(e.FileName)) editCache.Add(guid, ImageFromFile(e.FileName));
                }
            }
        }
        /// <summary>
        ...
        /// <param name="useEmbeddedThumbnails">UseEmbeddedThumbnails property of the owner control.</param>
        public void Add(Guid guid, string filename, DateTime dateModified, Size thumbSize, UseEmbeddedThumbnails useEmbeddedThumbnails)
        {
            lock (lockObject)
            {
                // Already cached?
        ...
        private Image ThumbnailFromFile(string fileName, DateTime dateModified, Size size, UseEmbeddedThumbnails useEmbeddedThumbnails, out bool wasThumbnailReadFromFile, out bool didErrorOccur)
        {
            ...
            else
            {
                try
                {
                    RetrieveItemThumbnailEventArgs eRetrieveItemThumbnailEventArgs = new RetrieveItemThumbnailEventArgs(fileName, dateModified);
        ...
        private void DoWork()
        {
            ...
            while (!Stopping && !stoppingBackgroundThreads)
            {
                ...
                while (queueFull && !Stopping && !stoppingBackgroundThreads)
                {
                    ...
                    // Is it outside visible area? If OnDemand, only read what is visible
                    /*
                    bool isvisible = true;
                    if (request != null && mCacheMode == CacheMode.OnDemand && !stoppingBackgroundThreads)
                    {
                        try
                        {
                            if (mImageListView != null && mImageListView.IsHandleCreated && !mImageListView.IsDisposed)
                        ...
                        {
                            Debug.WriteLine("DoWork 2/Is it outside visible area? If OnDemand, only read what is visible: " + ex.Message);
                            if (!Stopping) throw;
                        }
                    ...
                    */
                    ...
                    if (request != null && !stoppingBackgroundThreads)
                    {
                        ...
                        lock (lockObject)
                        {
                            if (!editCache.TryGetValue(guid, out editSource)) editSource = null;
                        }
                        if (editSource != null)
                        {
                        ...
                        */
                        ...
                        if (thumb == null)
                        {
                            ...
                            if (request.IsVirtualItem)
                            {
                                ...
                                if (e.ThumbnailImage != null) thumb = e.ThumbnailImage;
                            }
                            else
                            {
                            */
                                ...
                                if (!stoppingBackgroundThreads)
                                {
                                    ...
                                    thumb = ThumbnailFromFile(request.FileName, request.DateModified, request.Size, request.UseEmbeddedThumbnails, out wasThumbnailReadFromFile, out didThumbnailReadErrorOccur);
                                }
                            //}
                        }
                        ...
                        try
                        {
                            if (mImageListView != null && mImageListView.IsHandleCreated && !mImageListView.IsDisposed && mImageListView.Enabled && !stoppingBackgroundThreads)
                            {
                                if (thumb != null)
                                {
                                    if (!stoppingBackgroundThreads)
                                    {
                                        mImageListView.Invoke(new ThumbnailCachedEventHandlerInternal(
                                            mImageListView.OnThumbnailCachedInternal),
                                            guid, thumb, request.Size, (result == null),
                                            wasThumbnailReadFromFile, didThumbnailReadErrorOccur);
                                    }
                                }
                            }
                        }
                        ...
                        {
                            Debug.WriteLine("DoWork 2/Need to stop the thread before updates: " + ex.Message);
                            if (!Stopping) throw;
                        }
                        catch (InvalidOperationException ex)
                        {
                    ...
                    if (sw.ElapsedMilliseconds > 50)
                    {
                        ...
                        {
                            Debug.WriteLine("DoWork 2/Do we need a refresh?: " + ex.Message);
                            if (!Stopping) throw;
                        }
                        catch (InvalidOperationException ex)
                        {
                ...
                if (cleanupRequired)
                {
                    ...
                    {
                        Debug.WriteLine("DoWork 2/Clean up invisible items: " + ex.Message);
                        if (!Stopping) throw;
                    }
                    catch (InvalidOperationException ex)
                    {
                ...
                if (thumbnailCreated)
                {
                    ...
                    {
                        Debug.WriteLine("DoWork 2/thumbnailCreated: " + ex.Message);
                        if (!Stopping) throw;
                    }
                    catch (InvalidOperationException ex)
                    {

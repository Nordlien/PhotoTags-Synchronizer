...
{
	#region NetworkCompuersAndSharesHandler
	public class NetworkCompuersAndSharesHandler
	{
        #region 
        private Object computerNamesLock = new Object();
		private List<string> computerNames = new List<string>();
		public string[] ComputerNames
		{
			get
			{
				lock (computerNamesLock)
				{
		...
		public void ScanForComputers()
		{
			string ipBase = GetLocalHostIPAddress();
			...
			for (int i = 1; i < 255; i++)
			{
				...
				Ping ping = new Ping();
				ping.PingCompleted += new PingCompletedEventHandler(pingPingCompleted);
		...
		#region pingPingCompleted
		private void pingPingCompleted(object sender, PingCompletedEventArgs e)
		{
			...
			if (e.Reply != null && e.Reply.Status == IPStatus.Success)
			{
				...
				try
				{
					...
					hostName = DetermineComputerName(ip);
					if (!string.IsNullOrWhiteSpace(hostName))
					{
						lock (computerNamesLock)
						{
							if (!computerNames.Contains(hostName)) computerNames.Add(hostName);
						}
					}
				}
	...
	public class ShareCollection : ReadOnlyCollectionBase
	{
		...
		/// <param name="server">The server name</param>
		/// <param name="shares">The ShareCollection</param>
		protected static void EnumerateSharesNT64(string server, ShareCollection shares)
		{
			int level = 2;
			int entriesRead, totalEntries, nRet, hResume = 0;
			IntPtr pBuffer = IntPtr.Zero;
		...
		/// <param name="server">The server name</param>
		/// <param name="shares">The ShareCollection</param>
		protected static void EnumerateShares9x64(string server, ShareCollection shares)
		{
			int level = 50;
			int nRet = 0;
			ushort entriesRead, totalEntries;
		...
		protected static void EnumerateShares(string server, ShareCollection shares)
		{
			...
			{
				if (IsNT) EnumerateSharesNT64(server, shares);
				else EnumerateShares9x64(server, shares);
			}
			else
			{
				if (IsNT) EnumerateSharesNT(server, shares);
				else EnumerateShares9x(server, shares);
			}
		}
